...
- ✅ **Hardened RLS v3 regime** — `projects.owner_id = auth.uid()` owner-scoping; anon reads via sanitizing views (`projects_public`, `projects_client_public`, `client_feedback_public`) filtered to published versions; opaque embed resolution via `resolve_embed_project`. See [[03_Protocol/System_Protocols#`projects`|projects RLS]].
...
- ✅ **Archived-version retention pruning function** — `prune_archived_presentation_versions` (pg_cron-ready) + client `pruneArchivedVersions`; never touches draft/published rows. *(Scheduling on live Supabase tracked in [[backlog#🏗️ Future Infrastructure|backlog]].)*
...
- ✅ **Director Notes feature** (G6.5a–g) — completed and merged per project memory.
